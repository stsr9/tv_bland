...
export const EpisodeDetails = ({ episodeId }: Props) => {
	...
	useEffect(() => {
		...
		return () => {
			dispatch(episodeActions.clearDetails())
		}
	}, [dispatch, episodeId])
						...
						<div className="info flex-column">
							<div className="rating">
								...
								{episodeDetails.rating.average && (
									<span className="dtc">
										{calcRating(episodeDetails.rating.average)}/5
									</span>
								)}
							</div>
							<div className="title">
							...
							<div
								...
								dangerouslySetInnerHTML={{
									__html: DOMPurify.sanitize(episodeDetails.summary)
								}}
							/>
						</div>
					</div>
				...
				<div className="view-content">
					<div className="episode-meta-data flex">
						<div className="episode-info flex-column">
							...
							<MetaDataList wrapRows>
								<div>
									<label>{t('episodeDetails.streamedOn')}</label>
									<span>{episodeDetails.webChannel?.name}</span>
								</div>
								<div>
									<label>{t('episodeDetails.schedule')}</label>
									<span>{episodeDetails.schedule.days.join(', ')}</span>
								</div>
								<div>
									<label>{t('episodeDetails.status')}</label>
									<span>{episodeDetails.status}</span>
								</div>
								<div>
									<label>{t('episodeDetails.genres')}</label>
									<span>{episodeDetails.genres.join(', ')}</span>
								</div>
							</MetaDataList>
						</div>
